...
shape_predictor = face_recognition = detector = None

def comparisonLoadData():
...
def comparisonAddImage(path, list):
    image = io.imread(path)
    list.append(image)

def comparisonAetermineImage(image):
    determinator = detector(image)
    for i,j in enumerate(determinator):
        shape = shape_predictor(image, j)
    return face_recognition.compute_face_descriptor(image, shape)

def comparisonCompareImage(descriptor1, descriptor2):
...
model_graph = model_graph_def = None
label_map = categories = categories_dictionary = None
...
def findingLoadData(mode):
    global model_graph, categories, categories_dictionary
